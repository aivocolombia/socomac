#!/usr/bin/env python3
"""
Script para probar el nuevo método Upload + Send de WHAPI
"""
import requests
import base64
import json
import time

def test_upload_only():
    """
    Probar solo el upload de documento
    """
    print("🧪 Probando upload de documento...")
    print("=" * 60)
    
    backend_url = "https://socomac.onrender.com"
    
    try:
        response = requests.post(f"{backend_url}/whatsapp/test-upload-documento", timeout=30)
        
        print(f"📊 Status: {response.status_code}")
        
        if response.status_code == 200:
            result = response.json()
            print("✅ Upload exitoso:")
            print(f"📄 PDF tamaño: {result['pdf_tamaño']} bytes")
            print(f"📊 Base64 tamaño: {result['pdf_base64_tamaño']} caracteres")
            print(f"📤 Upload result: {result['upload_result']}")
        else:
            print(f"❌ Error: {response.status_code}")
            print(f"📄 Response: {response.text}")
            
    except Exception as e:
        print(f"❌ Error: {e}")

def test_full_send():
    """
    Probar envío completo con el nuevo método
    """
    print("\n🚀 Probando envío completo...")
    print("=" * 60)
    
    backend_url = "https://socomac.onrender.com"
    
    # Crear PDF de prueba
    pdf_content = """%PDF-1.4
1 0 obj
<<
/Type /Catalog
/Pages 2 0 R
>>
endobj

2 0 obj
<<
/Type /Pages
/Kids [3 0 R]
/Count 1
>>
endobj

3 0 obj
<<
/Type /Page
/Parent 2 0 R
/MediaBox [0 0 612 792]
/Contents 4 0 R
/Resources <<
/Font <<
/F1 5 0 R
>>
>>
>>
endobj

4 0 obj
<<
/Length 44
>>
stream
BT
/F1 12 Tf
72 720 Td
(Test PDF) Tj
ET
endstream
endobj

5 0 obj
<<
/Type /Font
/Subtype /Type1
/BaseFont /Helvetica
>>
endobj

xref
0 6
0000000000 65535 f 
0000000009 00000 n 
0000000058 00000 n 
0000000115 00000 n 
0000000274 00000 n 
0000000368 00000 n 
trailer
<<
/Size 6
/Root 1 0 R
>>
startxref
465
%%EOF"""
    
    pdf_bytes = pdf_content.encode('utf-8')
    pdf_base64 = base64.b64encode(pdf_bytes).decode('utf-8')
    
    payload = {
        "numero_telefono": "573172288329",  # Sin +57
        "pdf_base64": pdf_base64,
        "nombre_archivo": "test_recibo_123_2025-01-18.pdf",
        "metadata": {
            "numero_recibo": 123,
            "empresa": "SOCOMAC",
            "cliente": "David Cuellar",
            "valor": "10000",
            "concepto": "Mercancia nacional",
            "fecha": "2025-01-18"
        }
    }
    
    try:
        print(f"📱 Enviando a: {payload['numero_telefono']}")
        print(f"📄 Archivo: {payload['nombre_archivo']}")
        print(f"📊 PDF: {len(pdf_bytes)} bytes")
        
        response = requests.post(
            f"{backend_url}/whatsapp/enviar-pdf",
            json=payload,
            timeout=60  # Más tiempo para upload + send
        )
        
        print(f"📊 Status: {response.status_code}")
        
        if response.status_code == 200:
            result = response.json()
            print("✅ ¡PDF enviado exitosamente!")
            print(f"📱 Teléfono: {result['numero_telefono']}")
            print(f"📄 Archivo: {result['archivo']}")
            print(f"📏 Tamaño: {result['tamaño_mb']}MB")
            print(f"📤 Document ID: {result.get('whapi_response', {}).get('document_id', 'N/A')}")
            print(f"📨 Respuesta WHAPI: {result.get('whapi_response', {})}")
        else:
            print(f"❌ Error: {response.status_code}")
            try:
                error_detail = response.json()
                print(f"📄 Error detail: {error_detail}")
            except:
                print(f"📄 Error text: {response.text}")
                
    except Exception as e:
        print(f"❌ Error de conexión: {e}")

def test_debug_endpoints():
    """
    Probar endpoints de debug
    """
    print("\n🔍 Probando endpoints de debug...")
    print("=" * 60)
    
    backend_url = "https://socomac.onrender.com"
    
    # Test 1: Debug config
    try:
        response = requests.get(f"{backend_url}/whatsapp/debug-config", timeout=10)
        if response.status_code == 200:
            config = response.json()
            print("✅ Configuración:")
            print(f"🌐 Base URL: {config['config']['whapi_base_url']}")
            print(f"🔑 Token: {config['config']['whapi_token_configurado']}")
        else:
            print(f"❌ Error config: {response.status_code}")
    except Exception as e:
        print(f"❌ Error config: {e}")
    
    # Test 2: Test PDF
    try:
        response = requests.post(f"{backend_url}/whatsapp/test-pdf", timeout=10)
        if response.status_code == 200:
            result = response.json()
            print("✅ PDF test:")
            print(f"📄 Tamaño: {result['tamaño_bytes']} bytes")
            print(f"✅ Válido: {result['validacion']['valido']}")
        else:
            print(f"❌ Error PDF test: {response.status_code}")
    except Exception as e:
        print(f"❌ Error PDF test: {e}")

def main():
    """
    Ejecutar todas las pruebas
    """
    print("🚀 INICIANDO PRUEBAS DEL NUEVO MÉTODO UPLOAD + SEND")
    print("=" * 80)
    
    # Test 1: Debug endpoints
    test_debug_endpoints()
    
    # Test 2: Upload only
    test_upload_only()
    
    # Test 3: Full send
    print("\n" + "=" * 80)
    print("🤔 ¿Quieres probar el envío completo? (s/n)")
    print("=" * 80)
    
    # Para testing automático, descomenta esta línea:
    # test_full_send()
    
    print("\n" + "=" * 80)
    print("📊 PRUEBAS COMPLETADAS")
    print("=" * 80)
    print("✅ Revisa los logs en Render para más detalles")
    print("🔍 El nuevo método usa: Upload + Send (2 pasos)")

if __name__ == "__main__":
    main()
